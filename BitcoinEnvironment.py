...
from BitcoinTrainingActionsPositions import BitcoinTrainingActions as Actions
from BitcoinTrainingActionsPositions import BitcoinTrainingPositions as Positions
...
class BitcoinTrainingEnvironment(gym.Env):
    def __init__(self,
                 ...
                 path_plots,
                 trade_fee_bid=0.0,
                 trade_fee_ask=0.0) -> None:
        self.df = self._get_data(data_path)
    ...
    def mean_over_std(self, x: np.ndarray):
        std = np.std(x, ddof=1)
        mean = np.mean(x)
        return mean / std if std > 0 else 0

    def over_t_ratio(self, x: np.ndarray):
        sum_abs = np.sum(np.abs(x))
        sum = np.sum(x)
        return sum / sum_abs if sum_abs > 0 else 0

    def alr_output(self, x: np.ndarray):
        return np.mean(x)
    ...
    def get_sharpe_ratio(self):
        return self.mean_over_std(self.get_portfolio_log_returns())
    ...
    def get_sharpe_ratio_buy_and_hold(self):
        ...
        return self.mean_over_std(log_returns)
